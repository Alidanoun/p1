  ...
  client: {
    $transaction(arg, options) {
      if (typeof arg === 'function') {
        ...
        const contextStore = { ...context, inTransaction: true };
        return traceContext.run(contextStore, () => {
          return basePrisma.$transaction(wrappedFn, options);
        });
      }
      return basePrisma.$transaction(arg, options);
    }
  },
  query: {
    $allModels: {
      $allOperations({ model, operation, args, query }) {
        ...
        return new Promise((resolve, reject) => {
          try {
            ...
            const bridgeResolve = (val) => traceContext.run(context, () => resolve(val));
            const bridgeReject = (err) => traceContext.run(context, () => reject(err));
            ...
            if (args.skipSoftDelete) {
              ...
              query(args).then(bridgeResolve, bridgeReject);
              return;
            }
            ...
            if (branchId) {
              ...
              basePrisma.$transaction(async (tx) => {
                return traceContext.run(contextStore, async () => {
                  await tx.$executeRawUnsafe(`SELECT set_config('app.current_branch_id', $1, true)`, String(branchId));
                  return tx[model][operation](args);
                });
              }).then(bridgeResolve, bridgeReject);
              return;
            }
            ...
            const err = new Error(`[Zero-Trust] Access denied to RLS model '${model}'. Missing branch context or explicit admin bypass.`);
            console.error(err);
            throw err;
          } catch (err) {
            reject(err);
